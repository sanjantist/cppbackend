...
class Map {
   public:
    ...
    DogPos ProjectMovement(DogPos from, DogPos to) const;
   ...
   private:
    ...
    const RoadSegment* FindVerticalRoad(DogPos pos) const;
    const RoadSegment* FindHorizontalRoad(DogPos pos) const;
   ...
   private:
    ...
    std::unordered_map<int, std::vector<RoadSegment>> vertical_roads_;
    std::unordered_map<int, std::vector<RoadSegment>> horizontal_roads_;
};
