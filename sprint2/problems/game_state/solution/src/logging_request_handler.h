...
private:
    ...
    static std::string ExtractIp(const Req& req) {
        auto it = req.find(kClientIpHeader);
    ...
    static void LogResponse(const std::string& ip,
        ...
        if (ct_it == resp.end()) data["content_type"] = nullptr;
        else data["content_type"] = std::string(ct_it->value());
