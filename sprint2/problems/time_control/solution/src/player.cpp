...
Player::Player(Id id, std::string name, std::weak_ptr<GameSession> session)
    : id_(id), dog_(Dog(name)), session_(std::move(session)) {
    auto spawn_point = LockSession()->GetMap().GetRoads()[0].GetStart();
    dog_.SetPosition({(double)spawn_point.x, (double)spawn_point.y});
}
