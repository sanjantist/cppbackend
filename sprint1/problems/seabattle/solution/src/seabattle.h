...
public:
  ...
  State operator()(size_t x, size_t y) const { return Get(x, y); }
  ...
  bool IsLoser() const { return weight_ == 0; }

private:
  State &Get(size_t x, size_t y) { return field_[x + y * field_size]; }

  State Get(size_t x, size_t y) const { return field_[x + y * field_size]; }
