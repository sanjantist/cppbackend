...
std::vector<HotDog> PrepareHotDogs(int num_orders, unsigned num_threads) {
  ...
  // std::latch - объект синхронизации, который ведёт себя как счётчик обратного
  // отсчёта. num_waiting_threads потоков будут ожидать, пока этот счётчик не
  // обнулится
  ...
  for (int i = 0; i < num_orders; ++i) {
    ...
    net::dispatch(io, [&cafeteria, &hotdogs, &mut, i, start_time, &start,
                       num_waiting_threads] {
      std::osyncstream{std::cout} << "Order #" << i
                                  << " is scheduled on thread #"
                                  << std::this_thread::get_id() << std::endl;

      // Ждём, пока все рабочие потоки зайдут в эту функцию, чтобы убедиться,
      // что OrderHotDog будет вызван из нескольких потоков
