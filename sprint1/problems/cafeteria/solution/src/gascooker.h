...
Содержит несколько горелок (burner), которые можно асинхронно занимать (метод
UseBurner) и освобождать (метод ReleaseBurner). Если свободных горелок нет, то
запрос на занимание горелки ставится в очередь. Методы класса можно вызывать из
разных потоков.
*/
...
public:
  ...
  ~GasCooker() { assert(burners_in_use_ == 0); }

  // Используется для того, чтобы занять горелку. handler будет вызван в момент,
  // когда горелка занята Этот метод можно вызывать параллельно с вызовом других
  // методов
  void UseBurner(Handler handler) {
    // Выполняем работу внутри strand, чтобы изменение состояния горелки
    // выполнялось последовательно
    net::dispatch(
        strand_,
        // За счёт захвата self в лямбда-функции, время жизни GasCooker будет
        // продлено до её вызова
        [handler = std::move(handler), self = shared_from_this(),
         this]() mutable {
