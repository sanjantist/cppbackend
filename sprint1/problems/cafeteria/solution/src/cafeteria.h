   ...
   public:
    ...
    void OrderHotDog(HotDogHandler handler) {
        struct OrderState : std::enable_shared_from_this<OrderState> {
            explicit OrderState(net::any_io_executor ex, std::shared_ptr<HotDogHandler> h)
        ...
        net::post(strand_, [this, handler_ptr] {
            auto state = std::make_shared<OrderState>(strand_.get_inner_executor(), handler_ptr);
                    ...
                    net::dispatch(strand_, [this, state] {
                        ...
                        state->bread_timer.async_wait(net::bind_executor(
                            strand_, [this, state](const boost::system::error_code& ec) {
                                ...
                                if (!state->finished && state->bread_done && state->sausage_done) {
                                    try {
                                        HotDog hd{++next_hotdog_id_, state->sausage, state->bread};
