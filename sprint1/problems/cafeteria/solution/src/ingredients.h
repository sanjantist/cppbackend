...
public:
  ...
  void StartBake(GasCooker &cooker, Handler handler) {
    // Метод StartFry можно вызвать только один раз
    if (backing_start_time_) {
      throw std::logic_error("Frying already started");
    }

    // Запрещаем повторный вызов StartFry
    backing_start_time_ = Clock::now();
    ...
    cooker.UseBurner([self = shared_from_this(), handler = std::move(handler)] {
      // Запоминаем время фактического начала обжаривания
      self->backing_start_time_ = Clock::now();
      handler();
    });
  }
  ...
  void StopBaking() {
    if (!backing_start_time_) {
      throw std::logic_error("Backing has not started");
    }
    if (backing_end_time_) {
      throw std::logic_error("Backing has already stopped");
    }
    backing_end_time_ = Clock::now();
    // Освобождаем горелку
    gas_cooker_lock_.Unlock();
  }
  ...
  bool IsCooked() const noexcept {
    return backing_start_time_.has_value() && backing_end_time_.has_value();
  }

  // Возвращает продолжительность выпекания хлеба. Бросает исключение, если хлеб
  // не был испечён
  Clock::duration GetBakingDuration() const {
    if (!backing_start_time_ || !backing_end_time_) {
      throw std::logic_error("Bread has not been cooked");
    }
    return *backing_end_time_ - *backing_start_time_;
  }

private:
  int id_;
  GasCookerLock gas_cooker_lock_;
  std::optional<Clock::time_point> backing_start_time_;
  std::optional<Clock::time_point> backing_end_time_;
};
