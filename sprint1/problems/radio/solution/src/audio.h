...
class Recorder {
  ...
  void SaveBuffer(const void *pInput, ma_uint32 frameCount) {
    ...
    std::copy_n(reinterpret_cast<const char *>(pInput), size,
...
public:
  ...
  ~Recorder() { ma_device_uninit(&device_); }
...
class Player {
  ...
  void FillBuffer(void *pOutput, ma_uint32 frameCount) {
    ...
    std::copy_n(current_buffer_ + current_off_, size,
                reinterpret_cast<char *>(pOutput));
...
public:
  ...
  ~Player() { ma_device_uninit(&device_); }
